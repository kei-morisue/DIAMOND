...
public class RectangleClipper {
    ...
    public boolean clip(OriLine l) {
        if (Math.abs(l.p0.x - m_minX) < Constants.EPS
                && Math.abs(l.p1.x - m_minX) < Constants.EPS) {
            return false;
        }
        if (Math.abs(l.p0.x - m_maxX) < Constants.EPS
                && Math.abs(l.p1.x - m_maxX) < Constants.EPS) {
            return false;
        }
        if (Math.abs(l.p0.y - m_minY) < Constants.EPS
                && Math.abs(l.p1.y - m_minY) < Constants.EPS) {
            return false;
        }
        if (Math.abs(l.p0.y - m_maxY) < Constants.EPS
                && Math.abs(l.p1.y - m_maxY) < Constants.EPS) {
            return false;
        }
